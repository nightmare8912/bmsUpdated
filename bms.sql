...
INSERT INTO `balance` VALUES (14680650,10291),(19878772,7158),(39237768,-1),(107966489,-1),(143727254,10007.3),(171261237,10050),(205058549,506832),(227916089,-1),(261413554,280000),(281754829,-1),(324804399,13562.5),(488521480,-1),(527880378,63637.5),(549939802,100),(626881857,10802),(700960728,4031.2000000000007),(712938379,10000),(734745215,-1),(747833094,-1),(835112867,10000),(838717474,1210.5),(848652407,68531.5),(893716579,447907.12);
...
--
-- Temporary view structure for view `totalamtinfd_updated`
--

DROP TABLE IF EXISTS `totalamtinfd_updated`;
/*!50001 DROP VIEW IF EXISTS `totalamtinfd_updated`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `totalamtinfd_updated` AS SELECT 
 1 AS `accNum`,
 1 AS `amtDeposited`,
 1 AS `finalAmt`,
 1 AS `noOfActiveFD`,
...
--
-- Final view structure for view `totalamtinfd_updated`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `totalamtinfd_updated` AS select `s3`.`accNum` AS `accNum`,ifnull(sum(`f`.`amt`),0) AS `amtDeposited`,ifnull(sum(`f`.`finalamt`),0) AS `finalAmt`,ifnull(count(`f`.`accnum`),0) AS `noOfActiveFD`,`b`.`bal` AS `bal` from ((`signup3` `s3` join `balance` `b` on((`s3`.`accNum` = `b`.`accNum`))) left join `fd` `f` on(((`s3`.`accNum` = `f`.`accnum`) and (`f`.`status` = 'active')))) where (`s3`.`status` = 'active') group by `s3`.`accNum` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-10-18 19:26:06
